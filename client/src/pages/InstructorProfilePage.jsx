...
export default function InstructorProfilePage() {
  ...
  // Capitalize instructor name for display
  const displayName = instructorName.charAt(0).toUpperCase() + instructorName.slice(1).toLowerCase();
  ...
  const fetchInstructorSessions = async () => {
    try {
      ...
      // Filter sessions by instructor name (case-insensitive)
      const filtered = allSessions.filter(
        session => session.instructorName.toLowerCase() === instructorName.toLowerCase()
      );
  ...
  const handleCreateSession = async (event) => {
    ...
    try {
      const payload = {
        ...
        sessionName: sessionName.trim() || `${displayName}'s Game`,
        config: {
      ...
      navigate(`/manage/${data.code}`, { 
        state: { instructorName: displayName } 
      });
    } catch (err) {
      setErrorMessage(err.message);
    }
  ...
  return (
    <div className="app-shell">
      <div className="card">
        <header style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <Link to="/instructor" style={{ textDecoration: 'none' }}>
              <button style={{ fontSize: '0.875rem' }}>All Instructors</button>
            </Link>
          </div>
        </header>
        ...
        <section>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <h2 style={{ margin: 0 }}>Your Session History ({instructorSessions.length})</h2>
